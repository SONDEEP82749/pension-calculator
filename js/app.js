(function(){
  ...
  q('calculateBtn').addEventListener('click', ()=>{
    ...
    // Display result block and highlight age factor
  ...
  // Popup close + reset buttons
  q('closePopup').addEventListener('click', ()=>{ q('popupAd').style.display='none'; });
...
// ✅ Grand Total Calculation (added here)
let grandTotal = 0;
rows.forEach(r => {
  const val = parseFloat(r[1].replace(/,/g, '')) || 0;
  grandTotal += val;
